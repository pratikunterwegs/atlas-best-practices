...
sim_data <- mapply(function(p_outlier, ext_outlier, scale) {
  ...
  speed_res <- data.table(type = c("reference", "outliers", "cleaned"),
                          speed = c(speed, error_speed, clean_speed))
...
#sim_data[, `:=`(d_cleaned = abs(cleaned - reference),
 #               d_outliers = abs(outliers - reference))]

sim_data_long <- copy(sim_data)

sim_data_long <- melt(sim_data_long, 
                      id.vars = c("land_values", "ext_outlier", 
                                  "p_outlier", "scale"))

ggplot(sim_data_long)+
  geom_boxplot(aes(x = factor(land_values),
                   y = value,
                   col = variable))+
  facet_grid(scale ~ ext_outlier, scales = "free")
```
