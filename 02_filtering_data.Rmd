...
# add 100 outliers
data_copy <- do_add_outliers(data_copy, p_data = 0.005, std_dev = 0.05)
```
...
# save figure
ggsave(plot_figure, filename = "figures/fig_add_errors.png", dpi = 300,
...
# data outside a box
data_outside_bbox <- atlastools::atl_filter_bounds(data = data_copy,
                                                  x_range = c(-1, 0.2),
                                                  y_range = c(-1, 0.2),
                                                  remove_inside = TRUE)
```
...
```{r}
# data to plot
data_plot <- list(data_copy, data_inside_bbox, data_outside_bbox)
...
# add elements
plots[[1]] <- plots[[1]] + 
  annotate(xmin = 0.25, xmax = 0.75, ymin = 0.25, ymax = 0.75,
              fill = NA, col = "black", lty = 1, geom = "rect")+
  annotate(x = 0.25, y = 0.8, label = "(b)", 
           fontface = "bold", geom = "text", size = 4)

# add to plot
plots[[1]] <- plots[[1]] + 
  annotate(xmin = -0., xmax = 0.2, ymin = -0.1, ymax = 0.2,
              fill = NA, col = "black", lty = 1, geom = "rect")+
  annotate(x = 0, y = 0.25, label = "(c)", 
           fontface = "bold", geom = "text", size = 4)
...
data_filtered <- atl_filter_covariates(data_copy,
            filters = c("(in_speed < 0.029 & out_speed < 0.029) | angle < 40"))
```
...
plots <- lapply(data_plot, function(df) {
  ggplot()+
    ...
    geom_point(data = df,
               aes(x, y, 
                   col = (in_speed >= 0.029 & out_speed >= 0.029)),
               shape = ifelse((df$in_speed >= 0.029 & df$out_speed >= 0.029), 
                              16, 1),
               show.legend = F)+
...
```{r}
sim_data <- mapply(function(p_outlier, ext_outlier, scale, replicate) {
...
speed_data <- dcast(speed_data, 
      scale + p_outlier + ext_outlier + replicate ~ type, 
...
speed_data <- melt(speed_data[, !c("replicate", "cleaned", 
                                   "reference", "outliers")], 
                   id.vars = c("scale", "p_outlier", "ext_outlier"))

# summarise
speed_data <- speed_data[, list(mean = mean(value),
                                sd = sd(value)),
                         by = list(scale, p_outlier, ext_outlier, variable)]

ggplot(speed_data)+
  geom_pointrange(aes(factor(ext_outlier), mean - 100,
                      ymin = mean - sd - 100,
                      ymax = mean + sd - 100,
                      colour = scale,
                      group = interaction(scale, ext_outlier)),
                  position = position_dodge(width = 0.2))+
  geom_line(aes(factor(ext_outlier), mean - 100,
                      group = scale),
            position = position_dodge(width = 0.2))+
  theme_test()+
  theme(strip.background = element_rect(fill = "grey90",
