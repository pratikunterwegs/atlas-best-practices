...
sim_data <- mapply(function(p_outlier, ext_outlier) {
  # make simulated data
  this_data <- do_smoove_data()
  
  # extract 'true' movement metrics
  speed <- atl_get_speed(this_data, type = "in")
  
  # extract proportion of landscape used
  land_vals <- extract(landscape,
                       this_data[, c("x", "y")])
  
  land_vals <- as.data.table(table(landscape))
