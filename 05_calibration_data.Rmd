...
fig_data_bbox <-
  ...
  geom_path(data = data_raw,
            aes(x, y),
             col = "grey", alpha = 0.5, size = 0.3)+
  geom_point(data = data_raw[!data, on = c("x", "y")],
             aes(x, y),
             col = "red", alpha = 1, size = 2)+
...
fig_low_nbs <- ggplot()+
  ...
  geom_point(data = data_raw[!data, on = c("x", "y")],
            aes(x, y),
            shape = 4,
            col = "red", alpha = 0.5, size = 0.3)+
  geom_point(data = data,
             aes(x, y, col = NBS),
             alpha = 1, size = 0.2)+
  scico::scale_colour_scico(palette = "davos",
                         labels = scales::comma, end = 0.9)+
  ggthemes::theme_map()+
  coord_sf(crs = 32631)+
  labs(colour = "NBS")
...
fig_speed_outliers <-
  ...
  geom_path(data = data,
            aes(x, y),
             col = "grey", alpha = 0.5, size = 0.3)+
  geom_point(data = data,
             aes(x, y, col = speed_in),
             size = 0.2)+
  scico::scale_colour_scico(palette = "davos", 
                         direction = 1, 
                         limits = c(NA, speed_angle_thresholds[1]),
                         na.value = "red",
                         labels = scales::comma,
                         end = 0.9,
                         trans = "sqrt")+
...
data <- atl_filter_covariates(data = data,
            filters = c("(speed_in < 20 & speed_out < 20)"))
...
# use only residence or walking points
data <- data[residence == T | tID == "walking", ]
```
...
patch_res_unknown <- atl_res_patch(data, 
                                buffer_radius = 10,
                                lim_spat_indep = 100,
                                lim_time_indep = 30,
                                min_fixes = 3)
...
```{r}
# all patch figures
fig_patches <- mapply(function(raw_data, patch_sf, patch_summary) {
  ggplot()+
    geom_point(data = raw_data,
               aes(x, y, col = residence),
               size = 0.1)+
    geom_sf(data = patch_sf,
            fill = "yellow", col = "black",
            alpha = 0.5)+
    geom_segment(data = patch_summary,
              aes(x = shift(x_end, type = "lead"), 
                  y = shift(y_end, type = "lead"),
                  xend = shift(x_start), yend = shift(y_start)),
              col = "blue", size = 1)+
    scale_colour_manual(values = c("red", "blue"))+
    ggthemes::theme_map()
}, list(data_res, data), patch_sf_data, patch_summary_data,
SIMPLIFY = FALSE)
...
fig_patch_unknown <- 
  ...
  geom_sf(data = patch_sf_data[[2]],
          fill = "steelblue", col = NA)+
  geom_path(data = patch_summary_data[[2]],
            aes(x_median, y_median),
            col = "red", size = 0.2)+
    ggthemes::theme_map()
