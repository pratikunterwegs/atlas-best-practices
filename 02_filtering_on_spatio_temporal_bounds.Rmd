...
# save figure
ggsave("figures/fig_filter_bbox.png", dpi = 300, width = 8, height = 2)
```
...
```{r make_explicit_spatial_filter}
# read in data again
# read in the data
data <- data.table::fread("data/sim_data_sec_02_01.csv")

# make circular buffer around centroid
centroid <- c(median(data$x), median(data$y))

library(sf)
centroid <- st_point(centroid)
buffer <- st_buffer(centroid, dist = 10)
```
...
- benchmark time filter
- bmark spatial filter
- bmark time and space filter (both indep, eg. filter before release)
- bmark time filter (non-indep, both evald together)
