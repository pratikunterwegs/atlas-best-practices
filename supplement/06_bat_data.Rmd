...
ggplot(data_for_intervals)+
  ...
  scale_x_log10(
    labels = function(x) {sprintf("%ix", x)},
...
ggplot(data)+
  ...
  scale_x_log10(
    labels = function(x) {sprintf("%2.fm", x)}
  )+
  scale_y_sqrt()+
  coord_cartesian(
    xlim = c(0.5, 5e3)
...
After first plotting the individual tracks, we notice severe distortions, making pre-processing necesary](supplement/figures/fig_bat_raw.png)
...
This filtering reduced the data to an average of 10,447 positions per individual (78% of the raw data on average). However, some point outliers remain.](supplement/figures/fig_bat_filter_cov.png)
...
![Bat data with unrealistic speeds removed. Grey crosses show data that were removed. We calculated the incoming and outgoing speed of each position using `atl_get_speed`, and filtered out positions with speeds > 20 m/s using `atl_filter_covariates`, leaving 10,337 positions per individual on average (98% from the previous step).](supplement/figures/fig_bat_filter_speed.png)
...
![Bat data after applying a median smooth with a moving window $K$ = 5. Grey circles show data prior to smoothing. The smoothing step did not discard any data.](supplement/figures/fig_bat_smooth.png)

## Making residence patches

### Excluding transit points

First, we calculate the instantaneous speeds, and then apply a one-way median smooth to the speeds with $K$ = 5. This procedure is also applied to each individual *separately*.

```{r}
# split the data
data_residence <- split(data_smooth, data_smooth$TAG)
```

We apply this step as with the calibration data in the previous example.

```{r}
# get smoothed speeds per individual
data_residence <- lapply(data_residence, function(dt) {
  
  # first get the speed (in) between smoothed points
  dt[, c("speed_in", "speed_out") := list(
    atl_get_speed(dt, x = "X", y = "Y", time = "time"),
    NULL
  )]
  
  # then smooth the speeds in a new column called speed_smoothed
  dt[, speed_smoothed := runmed(speed_in, k = 5)]
  
  # filter for speeds less than 2 m/s
  dt = dt[speed_smoothed < 2, ]
...
ggsave(fig_bat_res_timeseries,
  filename = "supplement/figures/fig_bat_res_timeseries.png",
  dpi = 300,
  width = 6, height = 3
)
```
...
We apply the residence patch method, using the default argument values (`lim_spat_indep = 100` (metres), `lim_time_indep = 30` (minutes), and `min_fixes = 3`). We change the `buffer_radius` to 30 metres (twice the buffer radius is used, so points must be separated by 50m to be independent bouts).
