...
% authors go here:
\\
Pratik Rajan Gupte\textsuperscript{1, 2},
Christine E. Beardsworth\textsuperscript{2},
Orr Spiegel\textsuperscript{3},
Emmanuel Lourie\textsuperscript{4},
Sivan Toledo\textsuperscript{5},
Ran Nathan\textsuperscript{4},
...
\begin{figure}
    ...
    Users should begin by plotting their data to determine its status, for instance whether it needs to be susbet within a certain time interval, or whether there are obvious location errors.
    Users can then pre-process their data using the appropriate module, following the pre-processing steps corresponding to each module.
    ...
    Users may use the pipeline in full or just the module most appropriate for their data.
    In all cases, users are strongly encouraged to visualise their data and scan it for location errors as a first step, and to visualise it along the way.
    }
...
\begin{enumerate}
    ...
    \item After data access, we first encourage users to visualise their data to check for evident location errors \citep{slingsby2016}.
    \item Users can install \texttt{atlastools} using the \texttt{install\_github} function from the \texttt{R} package \texttt{remotes}.
    The package can be installed using the \texttt{R} command
    
    \texttt{remotes::install\_github("pratikunterwegs/atlastools")}.
    
    Package functions are prefixed `\texttt{atl\_}', and rely heavily on the \texttt{data.table} package (Dowle and Srinivasan 2020).
    ...
    Since ATLAS systems typically cover an area of a few hundred square kilometres, \texttt{atlastools} assumes a coordinate system in metres, and users must convert their geographic coordinates to metres.
    \item Users with a specific area or time of interest can use simple spatio-temporal range filters to select positions (see \textsc{Spatio-Temporal Filtering}).
    \item Next, users should reduce gross location errors by removing unreliable positions which may be identified by an error measure, or by the plausibility of associated movement metrics (see \textsc{Filtering to Reduce Location Errors}).
    ...
    \item Users who need uniformly thinned data can then thin by either aggregating or resampling (see \textsc{Smoothing and Thinning Data}).
    At this stage, the data are ready for a number of popular statistical treatments such as Hidden Markov Model-based classification \citep{michelot2016}.
    ...
    % Residence patches are spatio-temporally independent clusters of positions that represent prolonged residence of a tracked individual in an area, and which can indicate aspects of space use such resource exploitation \citep{barraquand2008, bijleveld2016, oudman2018}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/fig_02_bounds.png}
    ...
    \textbf{(b)} Tracks can be quickly filtered by spatial bounds ($0.5 \leq Y \leq 1.0$; dashed grey lines) using the \texttt{atlastools} function  \texttt{atl\_filter\_bounds}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/fig_04_median_smooth.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/fig_05_thinning.png}
...
% Here, high-throughput tracking every 1s -- 30s has its advantages: users may be permitted the assumption that an individual is truly absent from a location where it was not reported, and that at least during the tracking period, such absences are not simply artefacts of the sampling interval.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/fig_06_residence.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/fig_07_calib_residence_patch.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/fig_08_bats.png}
...
A. Ramesh and F.J. Weissing for discussions on error propagation;
J.R.L. Gismann and E. Gobbens for feedback that improved the manuscript; 
