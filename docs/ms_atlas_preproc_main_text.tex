...
\begin{enumerate}
    ...
    % Sophisticated methods that estimate both the error and the movement metrics are not easily scalable to massive datasets, and can be difficult to use for both novice and veteran animal ecologists.
    ...
    \item A pipeline for cleaning massive high-throughput datasets must balance ease of use, computational efficiency, and the ability to prepare movement data for a range of analyses.
...
High-throughput tracking is possible using GPS tags \citep[see recent examples in][]{strandburg-peshkin2015, papageorgiou2019, harel2016}, yet ‘reverse-GPS’ systems developed to track animals over land \citep{toledo2014, weiser2016, toledo2016,toledo2020, maccurdy2009, maccurdy2019} and in aquatic systems \citep{hussey2015, baktoft2019, baktoft2017,jung2015, aspillaga2021} routinely produce high-throughput tracking data at much lower costs.
...
\begin{enumerate}
    ...
    %  (see \textsc{Spatio-Temporal Filtering}).
...
For instance, in reverse-GPS ATLAS systems the number of base stations involved in each localisation is an indirect indicator of data quality, and positions localised using more receivers are usually more reliable \citep[the minimum required for an ATLAS localisation is 3; see][]{weiser2016}.
...
In our example for instance, the data with errors included would have to be thinned to \sfrac{1}{30}\textsuperscript{th} of its volume for the median speed to be correctly estimated; an undesirable step if the aim is fine-scale tracking (Fig. 4.c).
...
% In both aggregation and resampling, an extra column \texttt{time\_agg} is added which has a uniform difference between each element corresponding to the user-defined thinning interval.
...
Users aiming for such analyses can benefit from segmentation-clustering the data into spatio-temporally independent `residence patches', and link the attributes of movement segments (such as duration or distance), with environmental data \citep{bijleveld2016, oudman2018, barraquand2008}.
